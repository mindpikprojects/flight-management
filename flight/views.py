...
# CHART_DIR = os.path.join(BASE_DIR, 'media')
# import pandas as pd
...
#         # Create a DataFrame
#         ticket_data = pd.DataFrame(list(tickets.values()))
        
#         # Convert booking_date to datetime
#         ticket_data['booking_date'] = pd.to_datetime(ticket_data['booking_date'])
        
#         # Group by booking_date and count total bookings
#         bookings_per_date = ticket_data.groupby(ticket_data['booking_date'].dt.date).size()
        
#         # Render the DataFrame as a pie chart
#         plt.figure(figsize=(8, 8))
#         bookings_per_date.plot(kind='pie', autopct='%1.1f%%')
#         plt.title('Total Ticket Bookings per Date')
#         plt.ylabel('')
#         plt.legend(loc='best')
        
#         # Save the chart as an image
#         chart_path = 'media/pie_chart.png'  # Change the path as per your project structure
...
def dashboard(request):
    # item = Ticket.objects.all().values()
    # df = pd.DataFrame(item)
    # df1 = df.name.tolist()
    # df1 = df['flight'].to_list()
    # df = df['booking_date'].to_list()

    df1 = [1,21,0.3,4,5]
    # df1 = np.random.randint(100, size=(5))
    df = [10,20,30,40,50]
    # df = np.random.randint(200, size=(5))
    print(df1, df)
    mydict = {
        'df1':df1,
        'df':df
    }
    return render(request, 'flight/dashboard.html', context=mydict)
